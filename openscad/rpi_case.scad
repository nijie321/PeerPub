...
//    protrodingThickness=2.2;
    // front wall
    if(head ==  1){
        translate([0,(lcdWidthForScrewHolder/2 - (squareHeight/2)) + (squareHeight/2+(baseBoardThickness+1) /2) ,(width/2) - ((baseBoardThickness+1) /2)]) cube([lcdHeight+((baseBoardThickness+1)*2),baseBoardThickness+1,width],center=true);    
    }
    echo((lcdWidthForScrewHolder/2 - (squareHeight/2)) , lcdWidth, lcdWidth - ((lcdWidthForScrewHolder/2 - (squareHeight/2))))
    
    // end wall
    if(head == 0){
//    ((width/2)-((protrodingThickness)/2))    
                translate([0,-(((protrodingThickness) /2)+(lcdWidth/2)),width/2-(baseBoardThickness-1)]) cube([lcdHeight+((baseBoardThickness+1)*2),protrodingThickness,width ],center=true);
    }
...
module placeScrewHolder(x=(lcdHeight/2 - (squareWidth/2)) , y=(lcdWidthForScrewHolder/2 - (squareHeight/2)) , z=squareThickness * 1.5 + 15.6, rearEnd=false, flag=1) {
    ...
    if(rearEnd==true){
        if(flag == 1){
            translate([x,y,z])
            difference(){
                    cube([squareWidth+1,squareHeight+1,squareThickness],center=true);
                    color("blue") translate([1,-1,0]) cylinder(r=screwInnerRadius+.1,h=squareThickness,center=true);    
            }
        }
...
toTranslateZ = 1.5*depth + 1.5*(baseBoardThickness+1) + width/2 + 5.4;//2.5;
...
//placeCornerScrewHolder(x=toTranslateX,y=-toTranslateY,z=toTranslateZ - 2.5);
placeCornerScrewHolder(x=toTranslateX,y=(lcdWidthForScrewHolder/2 - (squareHeight/2)),z=toTranslateZ - 2.5);
    
placeCornerScrewHolder(x=-toTranslateX,y=-toTranslateY + 11,z=toTranslateZ - 2.5);        
//placeCornerScrewHolder(x=-toTranslateX,y=(lcdWidthForScrewHolder/2 - (squareHeight/2)),z=toTranslateZ - 2.5); 
...
color("green") translate([toTranslateX,-toTranslateYBottom, toTranslateZBottom + heightToScrew + 28.70]) rotate([90,0,0]) placeCornerScrewHolder();
color("green") translate([-toTranslateX,-toTranslateYBottom,toTranslateZBottom + heightToScrew + 28.70]) rotate([90,0,0]) placeCornerScrewHolder();       
}
...
module makeSideWall(sides=[1,0]){
    ...
    //lcdWidth
    if(sides == [1,0]){
        color("red") translate([ -( (lcdHeight/2) +((baseBoardThickness+1)/2) ),0,( (width - (baseBoardThickness-1) )/2) -2 - 1/2 ]) cube([baseBoardThickness+1,lcdWidth,width- baseBoardThickness - 1],center=true);
            
       }
    if(sides == [0,1]){
        color("blue") translate([ (lcdHeight/2) +((baseBoardThickness+1)/2),0,(width/2) - ( (baseBoardThickness-1) / 2) -2 - 1/2]) cube([baseBoardThickness+1,lcdWidth,width - baseBoardThickness - 1],center=true);
    }
}
...
slotToEdge = 40.5;
moveZAxis = 5;
color("red") translate([(lcdHeight/2) - ((baseBoardThickness+1)/2) + slotDepth,-screenWidth/2 + (slotWidth/2) + slotToEdge/2 + (5),(slotDepth/2) + (squareThickness/2) + heightToScrew + moveZAxis - 2]) cube([slotDepth+3,slotWidth+7,slotHeight+7],center=true);
}
...
module tempSlot(){
extraZ = 10;
    color("red") translate([0, (lcdWidth/2) + ((baseBoardThickness+1)/2)   ,((lcdHeight/4)/2) + ((baseBoardThickness+1) /2) + 10 + extraZ/2 ]) cube([ (lcdHeight+((baseBoardThickness+1)*30)) / 4 + 12 + 5,(baseBoardThickness+1) +10,lcdHeight/4 + extraZ],center=true);
//union(){
...
module touchsensor_holder(){
    difference(){
        cube([7.5,7.5,2.5],center=true);
        color("red") cylinder(d=2.67,h=2.5, center=true);
    }
}
...
union(){
  #baseWithScreen();
    difference(){
        ...
        union(){
//        translate([0,25,10]) makePlateSlot();
            translate([1.4,-18,12]) makePlateSlot();

        }
    }
    
    translate([-34.7,0,48]) union(){
        translate([0,10,0]) touchsensor_holder();
        translate([0,10 + 27.5,0]) touchsensor_holder();
    }
//  difference(){
//  makeSideWall([0,1]);
//  makePowerSlot();
//  }
    difference(){
        difference(){
            ...
            translate([24,65,55]) rotate([90,0,0]) cylinder(d=11.2,h=20,center=true);     
       } 
       tempSlot();
...
//makeRearendWall(0);
    //protrodingThickness
    translate([0,-(((protrodingThickness) /2)+(lcdWidth/2)),width/2-(baseBoardThickness-1) - 1.5]) cube([lcdHeight+((baseBoardThickness+1)*2) ,2.02,77.90 ],center=true);
 union(){
...
translate([toTranslateX,-toTranslateYBottom,toTranslateZBottom + heightToScrew]) color("blue") rotate([90,0,0]) cylinder(r=screwInnerRadius,h=depth/2+20,center=true);
    ...
    translate([toTranslateX,-toTranslateYBottom,toTranslateZBottom + heightToScrew + 28.7]) color("blue") rotate([90,0,0]) cylinder(r=screwInnerRadius,h=depth/2+20,center=true);
...
//    translate([toTranslateX,-toTranslateY,toTranslateZ]) translate([0,0,(depth/2)/2]) color("blue") cylinder(r=screwInnerRadius,h=depth/2 + 20,center=true);
    ...
    translate([-toTranslateX,-toTranslateY + 11,toTranslateZ]) translate([0,0,(depth/2)/2]) color("blue") cylinder(r=screwInnerRadius,h=depth/2 + 20,center=true);
...
//translate([0,0,20])
buttonDiameter = 13;
buttonRadius = buttonDiameter/2 + 1.2;
buttonOffset = 4.88;

module placeHoles(){
    color("red") translate([mountLength/2 - mountRadius - positionThreshold - 0.48,mountWidth/2 - mountRadius - positionThreshold,0]) cylinder(h=mountHoleDepth,r=mountRadius,center=true);
...
translate([0,toPlaceY + toPlaceYOffset, toPlaceZ]) rotate([0,90,0]) color("blue") cylinder(h=screwHeight,d=screwDiameter+1,center=true);
